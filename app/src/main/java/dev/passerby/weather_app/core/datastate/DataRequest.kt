...
inline fun <T> dataRequest(block: () -> T): DataState<T> {
    return try {
        ...
        when (e.code()) {
            400 -> {
                Timber.tag("data request EXCEPTION").e("data request FAILED,cause = SERVER ERROR, e = $e")
    ...
    } catch (e: RequestResponseException) {
        Timber.tag("data request EXCEPTION")
            .e(t = e, message = "data request FAILED,cause = REQUEST RESPONSE ERROR, e = $e")
        DataState.Error(type = Unknown)
    } catch (e: Exception) {
